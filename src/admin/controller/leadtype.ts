...
import { LeadType } from '../../types/collection/lead';

export const newLeadType = async (req: Request): Promise<ReplySuccess> => {
  try {
    ...
    const exists: number = await GetCount(CollectionName.LeadType, {
      name: { $eq: req.body.name },
    });
    if (!exists) {
      const insertData: LeadType = {
        name: req.body.name,
      ...
      if (insertedId && acknowledged) {
        return {
          ...
          response: { status: 1, message: 'Lead Type created successfully' },
        };
      } else {
        return {
          code: statusCode.InternalServer,
          response: { status: 0, message: 'Error in creating lead type' },
        };
      }
    } else {
      return {
...
export const updateLeadType = async (req: Request): Promise<ReplySuccess> => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return {
...
export const getLeadType = async (req: Request): Promise<ReplySuccess> => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return {
    ...
    const leadtype = (await GetOneDocument(
      CollectionName.LeadType,
    ...
    if (leadtype) {
      return {
        code: statusCode.Success,
        response: { status: 1, message: 'success', data: leadtype },
      };
    } else {
      return {
        code: statusCode.BadRequest,
...
export const getLeadTypeList = async (req: Request): Promise<ReplySuccess> => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return {
    ...
    const count: number = await GetCount(CollectionName.LeadType, query);
    ...
    const leadtype = (await GetDocument(
      CollectionName.LeadType,
      query,
    ...
    if (leadtype) {
      pagination.currentSize = leadtype.length;
      return {
        code: statusCode.Success,
        response: {
          status: 1,
          message: 'success',
          data: { list: leadtype, pagination },
        },
      };
    } else {
      return {
...
export const getAllActiveLeadType = async (
  req: Request
